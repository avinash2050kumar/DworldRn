...
class WorkScheduleScreen extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (isEmpty && !workEmpty) {
      return {
        workSchedules: Object.assign({}, props.workSchedules, {
          SelectedJobDays: props.workSchedules.JobDays[0],
          PreferDays: props.workSchedules.JobDays[0].Name,
          IsFri: false,
          IsMon: false,
          IsSat: false,
          IsSun: false,
  ...
  handleSubmit = async () => {
    const beforeValue = Object.assign({}, this.state.workSchedules, {
      PreferDays:
        this.state.workSchedules.PreferDays &&
        this.state.workSchedules.PreferDays === 'Custom Days'
          ? this.getDaysString()
          : this.state.workSchedules.PreferDays,
    });

    const afterValue = Object.assign(
      {},
      {
        workSheduleId: 0,
      ...
      <ScrollView
        ...
        <Screen style={{backgroundColor: theme.white}}>
          ...
          <StyledText style={{marginBottom: 15}}>
            Enter details of how you want to work
          </StyledText>
          {!this.isEmpty(workSchedules) && (
            <View>
              <Selector
                ...
                items={
                  workSchedules.ShitTypeData
                    ? workSchedules.ShitTypeData.map(shift => {
                        return {
                          ...shift,
                          value: shift.Name,
                        };
                ...
                onChange={value => {
                  let i = 0;
                  this.state.workSchedules.ShitTypeData.map((val, index) => {
                  ...
                  this.setState(
                    update(this.state, {
                      workSchedules: {
                        ShitType: {
                          $set: this.state.workSchedules.ShitTypeData[i],
                        },
                      },
                    }),
                  );
              ...
              <Selector
                ...
                items={
                  workSchedules.JobTypeData
                    ? workSchedules.JobTypeData.map(jobtype => {
                        return {
                          ...jobtype,
                          value: jobtype.Name,
                        };
                ...
                onChange={value => {
                  let i = 0;
                  this.state.workSchedules.JobTypeData.map((val, index) => {
                    if (val.Name === value) i = index;
                  });
                  this.setState(
                    update(this.state, {
                      workSchedules: {
                        JobType: {
                          $set: this.state.workSchedules.JobTypeData[i],
                        },
                      },
                    }),
                  );
              ...
              <Selector
                theme="dropdown" // Default: 'simple'
                items={
                  workSchedules.JobDays
                    ? workSchedules.JobDays.map(shift => {
                        return {
                          ...shift,
                          value: shift.Name,
                        };
                ...
                labelKey="value" // Default: 'label'
                defaultValue={
                  workSchedules.SelectedJobDays
                    ? workSchedules.SelectedJobDays.Name
                    : ''
                } // Set default value
                ...
                onChange={value => {
                  let i = 0;
                  this.state.workSchedules.JobDays.map((val, index) => {
                    if (val.Name === value) i = index;
                  });
                  this.setState(
                    update(this.state, {
                      workSchedules: {
                        SelectedJobDays: {
                          $set: this.state.workSchedules.JobDays[i],
                        },
                        PreferDays: {
                          $set: this.state.workSchedules.JobDays[i].Name,
                        },
                      },
                    }),
                  );
              ...
              />
              {workSchedules.PreferDays &&
                workSchedules.PreferDays === 'Custom Days' && (
                  <View>
