...
export const setVehicleCompany = () => async dispatch => {
  try {
    const res = await client.main.getVehicleCompany();
    dispatch({type: SET_LEASE_VEHICLE_COMPANY, payload: res.data});
  } catch (e) {
    // dispatch(setAppMessage('Error', 'Something went wrong', 'danger'));
  }
};

export const setOwnerVehicleCompany = () => async dispatch => {
  try {
    const res = await client.main.getVehicleCompany();
    dispatch({type: SET_OWNER_VEHICLE_COMPANY, payload: res.data});
  } catch (e) {
    // dispatch(setAppMessage('Error', 'Something went wrong', 'danger'));
  }
};

export const driverAddNewVehicle = text => async dispatch => {
  try {
    const res = await client.main.driverAddNewVehicle(text);
    dispatch({
      type: DRIVER_ADD_NEW_VEHICLE,
      payload: res.data,
    });
    return res.data;
  } catch (e) {
    dispatch(setAppMessage('Error', 'Unable to add Vehicle', 'danger'));
  }
};

export const ownerAddNewVehicle = text => async dispatch => {
  try {
    const res = await client.main.driverAddNewVehicle(text);
    dispatch({
      type: OWNER_ADD_NEW_VEHICLE,
      payload: res.data,
    });
    return res.data;
  } catch (e) {
...
export const leaseAddNewVehicle = text => async dispatch => {
  try {
    const res = await client.main.driverAddNewVehicle(text);
    dispatch({
      type: LEASE_ADD_NEW_VEHICLE,
      payload: res.data,
    });
    return res.data;
  } catch (e) {
...
export const postWorkSchedule = payload => async dispatch => {
  try {
    ...
    dispatch(setAppMessage('Error', 'Unable to save Work Schedule', 'danger'));
  }
};
/*=============================================*/
...
export const saveDriverHourlyInfo = values => async dispatch => {
  try {
    ...
    res.status === 204 &&
      dispatch(
        setAppMessage('Error', 'Unable to save Driver hourly values', 'danger'),
      );
    return res;
  } catch (e) {
    dispatch(setAppMessage('Error', 'Check Network Connection', 'danger'));
...
export const saveDriverWeeklyInfo = values => async dispatch => {
  try {
    ...
    res.status === 204 &&
      dispatch(
        setAppMessage('Error', 'Unable to save Driver Weekly values', 'danger'),
      );
    return res;
  } catch (e) {
    dispatch(setAppMessage('Error', 'Check Network Connection', 'danger'));
...
export const saveDriverTripPayInfo = values => async dispatch => {
  try {
    ...
    res.status === 204 &&
      dispatch(
        setAppMessage('Error', 'Unable to save Driver Trip values', 'danger'),
      );
    return res;
  } catch (e) {
    dispatch(setAppMessage('Error', 'Check Network Connection', 'danger'));
...
export const driverApproveJobOffer = (
  toClientOffer,
  jobId,
) => async dispatch => {
  try {
...
export const SaveProfile = value => async dispatch => {
  try {
    ...
    if (res.status === 200) {
      dispatch(
        setAppMessage('Success', 'Successfully saved your details', 'success'),
      );
...
export const postOwnerDriverJob = () => async dispatch => {
  try {
    ...
    res.status === 200
      ...
      : dispatch(
          setAppMessage('Error', 'Unable to save Job for drivers', 'danger'),
        );
    return res.status;
  } catch (e) {
    dispatch(setAppMessage('Error', 'Check Network Connection', 'danger'));
...
export const postOwnerVehicleFirm = () => async dispatch => {
  try {
    ...
    res.status === 200
      ...
      : dispatch(
          setAppMessage('Error', 'Unable to save Firm for drivers', 'danger'),
        );
    return res.status;
  } catch (e) {
    dispatch(
...
export const saveExperienceAndDl = payload => async dispatch => {
  try {
    ...
    res.status === 200
      ? dispatch(
          setAppMessage('Success', 'Successfully Save your Dl Info', 'success'),
        )
      : dispatch(setAppMessage('Error', 'Unable to save Dl Info', 'danger'));
    return res;
  } catch (e) {
    dispatch(
      setAppMessage(
